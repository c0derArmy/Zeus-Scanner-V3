...
try:
    ...
    print("Warning: pyvirtualdisplay module not found. Headless mode will be disabled.")
...
def get_urls(query, url, verbose=False, **kwargs):
    ...
    if DISPLAY_AVAILABLE:
        ...
        logger.warning(set_color(
            "pyvirtualdisplay not available, continuing without headless mode",
            level=30
        ))
        ff_display = None
    ...
    browser.close()
    if DISPLAY_AVAILABLE and ff_display is not None:
        ff_display.stop()
    return retval
