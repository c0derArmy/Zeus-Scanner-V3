    ...
    def set_browser(self):
        ...
        try:
            if not self.tor:
                ...
                if self.__set_proxy():
                    options.add_argument(f"--proxy-server={self.__set_proxy()}")
            ...
            if not self.tor:
                ...
                options = webdriver.FirefoxOptions()
                options.profile = profile
                service = webdriver.FirefoxService(executable_path="/home/devil/Desktop/Zeus-Scanner/bin/drivers/geckodriver")
