...
# clone link
CLONE = "https://github.com/shiva345-star/zeus-scanner.git"

# issue link
ISSUE_LINK = "https://github.com/shiva345-star/zeus-scanner/issues"
...
def run_attacks(url, **kwargs):
    ...
    use_wapiti = kwargs.get("use_wapiti", False)
    use_nikto = kwargs.get("use_nikto", False)
    ...
    if question.lower().startswith("y"):
        if sqlmap:
            ...
            try:
                ...
                if NMAP_AVAILABLE:
                    ...
                    logger.error("Nmap is not available, skipping port scan")
            except (ImportError, AttributeError):
                logger.error("Failed to import nmap module, skipping port scan")
        elif admin:
            ...
            from lib.core.xsser_integration import run_xsser_scan
            logger.info(set_color(
                "using XSSer tool for XSS vulnerability scanning"
            ))
            run_xsser_scan(
                url.strip(), 
                verbose=verbose, 
        ...
        if use_nuclei:
            ...
            if nuclei_question.lower().startswith("y"):
                try:
                    ...
                    nuclei_results = nuclei.comprehensive_scan(url.strip())
        ...
        # Run Wapiti scan if enabled
        if use_wapiti:
            wapiti_msg = "would you like to run Wapiti vulnerability scan on '{}'".format(url)
            if not batch:
                wapiti_question = lib.core.common.prompt(wapiti_msg, opts="yN")
            else:
                wapiti_question = lib.core.common.prompt(wapiti_msg, opts="yN", default="y")
            
            if wapiti_question.lower().startswith("y"):
                try:
                    from lib.integrations.wapiti_integration import WapitiIntegration
                    import tempfile
                    logger.info(set_color("starting Wapiti vulnerability scan", level=25))
                    wapiti = WapitiIntegration()
                    with tempfile.NamedTemporaryFile(suffix='.json', delete=False) as tmp:
                        output_file = tmp.name
                    wapiti_results = wapiti.comprehensive_scan(url.strip(), output_file)
                    
                    print(f"\n{set_color('Wapiti Scan Results:', level=25)}")
                    print(f"Total Vulnerabilities: {wapiti_results.get('total_vulnerabilities', 0)}")
                except ImportError:
                    logger.error(set_color("Wapiti integration not available", level=40))
                except Exception as e:
                    logger.error(set_color(f"Wapiti scan failed: {str(e)}", level=40))
        
        # Run Nikto scan if enabled
        if use_nikto:
            nikto_msg = "would you like to run Nikto web server scan on '{}'".format(url)
            if not batch:
                nikto_question = lib.core.common.prompt(nikto_msg, opts="yN")
            else:
                nikto_question = lib.core.common.prompt(nikto_msg, opts="yN", default="y")
            
            if nikto_question.lower().startswith("y"):
                try:
                    from lib.integrations.nikto_integration import NiktoIntegration
                    logger.info(set_color("starting Nikto web server scan", level=25))
                    nikto = NiktoIntegration()
                    # Port and SSL are auto-detected from URL
                    nikto_results = nikto.full_scan(url.strip())
                    
                    if nikto_results.get('success'):
                        print(f"\n{set_color('Nikto Scan Results:', level=25)}")
                        print(f"Target: {url.strip()}")
                        print(f"Port: {nikto_results.get('port', 'auto-detected')}")
                        print(f"SSL: {nikto_results.get('ssl', 'auto-detected')}")
                        print(f"Findings: {len(nikto_results.get('findings', []))}")
                        
                        # Display findings
                        for i, finding in enumerate(nikto_results.get('findings', [])[:10], 1):
                            print(f"  {i}. {finding.get('finding', 'N/A')}")
                        
                        if len(nikto_results.get('findings', [])) > 10:
                            print(f"  ... and {len(nikto_results.get('findings', [])) - 10} more findings")
                    else:
                        logger.error(set_color(f"Nikto scan failed: {nikto_results.get('error', 'Unknown error')}", level=40))
                        
                except ImportError:
                    logger.error(set_color("Nikto integration not available", level=40))
                except Exception as e:
                    logger.error(set_color(f"Nikto scan failed: {str(e)}", level=40))
    else:
        logger.warning(set_color(
