...
try:
    ...
    PATCH_ID = "N/A"  # Not a git repository or git not installed

# clone link
CLONE = "https://github.com/c0derArmy/Zeus-Scanner-V3.git"

# issue link
ISSUE_LINK = "https://github.com/c0derArmy/Zeus-Scanner-V3/issues"
...
def get_latest_log_file(log_path):
    ...
    """
    file_list = [f for f in glob.glob(log_path + "/*") if os.path.isfile(f)]
    try:
...
def find_application(application, opt="path"):
    ...
    conf_parser.read_string(read_conf)
...
def fix_log_file(logfile=get_latest_log_file(CURRENT_LOG_FILE_PATH)):
    ...
    """
    retval = ""
    escape_seq_regex = re.compile(r"\033\[\d+[*m]")
...
def run_attacks(url, **kwargs):
    """
    run the attacks if any are requested — supports running multiple attacks sequentially
    """
    nmap = kwargs.get("nmap", False)
    sqlmap = kwargs.get("sqlmap", False)
    ...
    use_nuclei = kwargs.get("use_nuclei", False)
    use_xsstrike = kwargs.get("use_xsstrike", False)
    sploit_scan = kwargs.get("sploit_scan", None)
    cve_verify = kwargs.get("cve_verify", False)
    autonomous = kwargs.get("autonomous", False)

    if threads is not None and threads > MAX_THREADS:
        threads = check_thread_num(threads, batch=batch)

    url = url.strip()  # Always strip whitespace/newlines from URL
    ...
    if question.lower().startswith("y"):
        # Run all enabled attacks sequentially
        if sqlmap:
            try:
                logger.info(set_color("starting SQLi scan with sqlmap", level=25))
                from lib.attacks import sqlmap_scan
                sqlmap_scan.sqlmap_scan_main(
                    url, verbose=verbose,
                    opts=create_arguments(sqlmap=True, sqlmap_args=sqlmap_arguments, conf=conf_file),
                    auto_start=auto_start)
            except Exception as e:
                logger.error(set_color(f"SQLi scan failed: {str(e)}", level=40))

        if nmap:
            try:
                logger.info(set_color("starting port scan with nmap", level=25))
                from lib.attacks import nmap_scan
                from lib.attacks.nmap_scan import NMAP_AVAILABLE
                url_ip_address = replace_http(url)
                if NMAP_AVAILABLE:
                    ...
                    logger.error(set_color("Nmap is not available, skipping port scan", level=40))
            except (ImportError, AttributeError) as e:
                logger.error(set_color(f"Failed to import nmap module: {str(e)}", level=40))

        if admin:
            try:
                logger.info(set_color("starting admin panel search", level=25))
                ...
                logger.error(set_color(f"Admin panel search failed: {str(e)}", level=40))

        if xss:
            try:
                logger.info(set_color("starting XSS vulnerability scan with XSStrike", level=25))
                ...
                logger.error(set_color(f"XSS scan failed: {str(e)}", level=40))

        if whois:
            try:
                logger.info(set_color("starting WhoIs lookup", level=25))
                from lib.attacks.whois_lookup.whois import whois_lookup_main
                whois_lookup_main(
                    url, verbose=verbose, timeout=timeout
                )
            except Exception as e:
                logger.error(set_color(f"WhoIs lookup failed: {str(e)}", level=40))

        if clickjacking:
            try:
                logger.info(set_color("starting clickjacking scan", level=25))
                ...
                logger.error(set_color(f"Clickjacking scan failed: {str(e)}", level=40))

        # Run Nuclei scan if enabled
        if use_nuclei:
            nuclei_msg = "would you like to run Nuclei vulnerability scan on '{}'".format(url)
            if not batch:
                nuclei_question = lib.core.common.prompt(nuclei_msg, opts="yN")
            else:
                nuclei_question = lib.core.common.prompt(nuclei_msg, opts="yN", default="y")

            if nuclei_question.lower().startswith("y"):
                try:
                    from lib.integrations.nuclei_integration import NucleiIntegration
                    logger.info(set_color("starting Nuclei vulnerability scan", level=25))
                    nuclei = NucleiIntegration()
                    nuclei_results = nuclei.comprehensive_scan(url)

                    print(f"\n{set_color('Nuclei Scan Results:', level=25)}")
                    print(f"Total Vulnerabilities: {nuclei_results.get('total_vulnerabilities', 0)}")

                    # Show findings
                    for finding in nuclei_results.get('findings', [])[:10]:
                        print(f"  [{finding.get('severity', 'info').upper()}] {finding.get('template_id', 'unknown')}")
                except ImportError:
                    logger.error(set_color("Nuclei integration not available", level=40))
                except Exception as e:
                    logger.error(set_color(f"Nuclei scan failed: {str(e)}", level=40))

        # Run SploitScan CVE scan if enabled
        if sploit_scan:
            try:
                from lib.cve_exploit.sploitscan_integration import scan_cve_main
                logger.info(set_color(f"Running SploitScan for CVE: {sploit_scan}", level=25))
                cve_result = scan_cve_main(
                    sploit_scan,
                    verify=cve_verify,
                    target=url,
                    verbose=verbose,
                ...
                if cve_result.get('cvss_score'):
                    logger.warning(set_color(
                        f"CVE {sploit_scan} - CVSS: {cve_result.get('cvss_score')} | Severity: {cve_result.get('severity', 'UNKNOWN')}",
                        level=35
                    ))
                if cve_result.get('exploits'):
                    logger.warning(set_color(
                        f"Found {len(cve_result['exploits'])} exploits for {sploit_scan}",
                        level=35
                    ))
            except Exception as e:
                logger.error(set_color(f"SploitScan failed: {str(e)}", level=40))

        # Run Autonomous Pentester if enabled
        if autonomous:
            try:
                from lib.mcp_server import run_autonomous_scan
                logger.info(set_color(
                    f"Starting AUTONOMOUS security audit on {url}",
                    level=25
                ))
                audit_result = run_autonomous_scan(url, verbose=verbose)

                print(f"\n{set_color('=== Autonomous Audit Results ===', level=25)}")
                summary = audit_result.get('summary', {})
                print(f"Total Findings: {summary.get('total_findings', 0)}")
                print(f"Total CVEs: {summary.get('total_cves', 0)}")
                print(f"Verified Exploits: {summary.get('verified_exploits', 0)}")

            except Exception as e:
                logger.error(set_color(f"Autonomous audit failed: {str(e)}", level=40))
