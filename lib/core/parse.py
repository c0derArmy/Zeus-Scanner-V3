...
class ZeusParser(OptionParser):
    ...
    def cmd_parser():
        ...
        """
        parser = OptionParser(usage="./zeus.py -d|r|l|f|u DORK|FILE|URL [ATTACKS] [--OPTS]")
        ...
        mandatory.add_option("-b", "--blackwidow", dest="spiderWebSite", metavar="URL",
                             help="[DEPRECATED - Use -u with --use-katana] Spider a single webpage for all available URL's")
        ...
        mandatory.add_option("-u", "--url", dest="singleTargetURL", metavar="URL",
                             help="Test a single URL without crawling (direct attack on one target)")
        ...
        attacks.add_option("-x", "--xss-scan", dest="runXsserScan", action="store_true",
                           help="Run XSS vulnerability scan using XSSer tool (120+ payloads)")
        ...
        attacks.add_option("--check-waf", dest="checkWafOnly", action="store_true",
                           help="Check for WAF/IDS/IPS protection using wafw00f (120+ signatures)")
        ...
        attacks.add_option("--xsser-args", dest="xsserArguments", metavar="XSSER-ARGS",
                           help="Pass the arguments to send to XSSer within quotes & "
                                "separated by a comma. IE 'threads 10, Doo, timeout 30'")
        ...
        integration_group = OptionGroup(parser, "AI Tool Integration Options",
                                      "Integration with AI-powered penetration testing tools")
        ...
        search_items.add_option("--use-katana", dest="useKatana", action="store_true",
                                help="Use Katana web crawler instead of default Blackwidow (faster, JS-aware)")

        search_items.add_option("--katana-depth", dest="katanaDepth", metavar="DEPTH", type=int, default=3,
                                help="Set Katana crawl depth (default: 3)")
