    ...
    if robots:
        ...
        if len(interesting) > 0:
            ...
            if to_display.lower().startswith("y"):
                # Ensure data is string for display
                display_data = data if isinstance(data, str) else data.decode('utf-8', errors='ignore')
                print(
                    "{}\n{}\n{}".format(
                        data_sep, display_data, data_sep
                    )
                )
...
def check_for_admin_page(url, exts, protocol="http://", **kwargs):
    ...
    for ext in exts:
        ...
        # Silent scanning - only show progress occasionally
        if verbose or (exts.index(ext) + 1) % 20 == 0:  # Show every 20th attempt
            lib.core.settings.logger.debug(lib.core.settings.set_color(
                "testing admin path {} ({}/{})".format(ext, exts.index(ext) + 1, len(exts)), level=10
            ))
        
        try:
            urlopen(true_url, timeout=5)
            # Success message - always show this
            print(f"{lib.core.settings.set_color('[SUCCESS]', level=40)} Admin panel found: {lib.core.settings.set_color(true_url, level=30)}")
            lib.core.settings.logger.info(lib.core.settings.set_color(
            ...
            if verbose:
                if "Access Denied" in str(e):
                    print(f"{lib.core.settings.set_color('[POSSIBLE]', level=30)} Restricted admin panel: {lib.core.settings.set_color(true_url, level=35)}")
                ...
                # Remove individual failure messages
            # Silent failure - no output for 404s unless verbose
...
def __load_extensions(filename="{}/etc/text_files/link_ext.txt"):
    ...
    with open(filename.format(os.getcwd())) as ext:
        return [line.strip() for line in ext.readlines()]
