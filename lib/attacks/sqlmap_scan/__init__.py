...
class SqlmapHook(object):
    ...
    def start_scan(self, api_id, opts=None):
        ...
        # Default API options to improve scan coverage
        data_dict = {
            "url": self.to_scan,
            "forms": True,
            "crawlDepth": 2,
        ...
        # Ensure post_data is bytes for Python 3
        if isinstance(post_data, str):
            post_data = post_data.encode('utf-8')
        req = Request(start_scan_url, data=post_data, headers=self.headers)
        ...
        while current_status == "running":
            ...
            for i in range(0, len(log_json["log"])):
                if log_json["log"][i]["message"] in already_displayed:
                    ...
                    # Colorful SQLMap log output
                    level = log_json["log"][i]["level"]
                    message = log_json["log"][i]["message"]
                    time_stamp = log_json["log"][i]["time"]
                    ...
                    # Highlight important SQLMap messages
                    if "vulnerable" in message.lower() or "injection" in message.lower():
                        message_color = 40
                    elif "parameter" in message.lower() and "testable" in message.lower():
                        message_color = 30
                    elif "target url" in message.lower() or "testing" in message.lower():
                        message_color = 25
                    else:
                        message_color = 20
                    
                    print(f"{lib.core.settings.set_color('[SQLMAP]', level=35)} "
                          f"{lib.core.settings.set_color(f'[{time_stamp} {level}]', level=level_color)} "
                          f"{lib.core.settings.set_color(message, level=message_color)}")
                    
                    # Special highlighting for vulnerability discoveries
                    if "vulnerable" in message.lower():
                        print(lib.core.settings.set_color("!" * 70, level=40))
                        print(lib.core.settings.set_color("         SQL INJECTION VULNERABILITY CONFIRMED!", level=40))
                        print(lib.core.settings.set_color("!" * 70, level=40))
